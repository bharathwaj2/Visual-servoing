...
def generate_launch_description():
    ...
    robot_description_config = xacro.process_file(
        os.path.join(
            get_package_share_directory('piper_arm_description'),
            'urdf',
            'piper_arm_robot.xacro',
        )
    )
    ...
    robot_description_semantic_path = os.path.join(
        get_package_share_directory('piper_arm_moveit_config'),
        'config',
        'piper_arm.srdf',
    )
    try:
    ...
    # kinematics yaml
    init_pos_yaml_path = os.path.join(
        get_package_share_directory('piper_arm_moveit_config'),
        'config',
        'initial_positions.yaml',
    )
    with open(init_pos_yaml_path, 'r') as file:
        init_pos_yaml = yaml.safe_load(file)
